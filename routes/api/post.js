...
router.put('/unlike/:id', auth(), async(req, res) => {
	try{
		...
		const removeIndex = post.likes.map(like => like.user.toStrong()).indexOf(req.user.id);
...
router.delete('/comment/:id/:comment_id', auth(), 
	async (req, res) => {
		...
		// Check user
		if(comment.user.toString() !== req.user.id && !user.id.roles.includes("editor") && !user.id.roles.includes("admin")){
			return res.status(401).json({msg: 'User not authorized'});
		}
